            ...
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form @submit.prevent="submit" class="space-y-4 md:space-y-6" >
                    <div>
                        ...
                        <input v-model="data.username" type="text" name="username" id="username" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="abcdef" required="">
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <input type="password" name="password" id="check-password" placeholder="••••••••" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="">
                    </div>
...
import  {useRouter} from 'vue-router'
    ...
    //     if (!response.ok) {
    //         // Si la réponse du serveur n'est pas réussie (par exemple, 400 Bad Request),
    //         // on lance une erreur avec le message renvoyé par le serveur
    //         const errorMessage = await response.text();
    //         throw new Error(errorMessage);
    //     }

    //     // Si la requête est réussie, on redirige vers '/login'
    //     await router.push('/login');
    // } catch (error) {
    //     // En cas d'erreur, on affiche le message d'erreur
    //     console.error('Erreur lors de la requête:', error.message);
    //     // Vous pouvez également choisir de gérer l'erreur d'une autre manière, par exemple en affichant un message d'erreur à l'utilisateur
    // }
